...
<body>
    ...
    <div id="main">
        <nav class="breadcrumb mt-5 border-none border-start border-5 border-danger shadow-sm bg-white py-1 px-5">
            <span class="breadcrumb-item active h3" aria-current="page"><b>Organized Planning Approach</b></span>
        </nav>
    ...
    <script>
        fetch(`../api/select_files.php?area=Planning`)
            .then(res => res.json())
            .then(data => {
                console.log(data);
                data.forEach(file => {
        ...
        $(document).on('click', '#upload', function(e) {
            ...
            if (fileType === 'pdf') {
                ...
                if (file.size > 500 * 1024 * 1024) {
                    $('#upload').html('Upload');
                    alert('PDF file size exceeds 500MB limit.');
                    return;
                }
            } else if (fileType === 'video') {
                const allowedVideoTypes = ['video/mp4', 'video/webm', 'video/ogg'];
